...
public class ReportWriter {	
	...
	private static final String serviceNamePlaceholder = "<!-- ServiceName -->";
	...
	public static void createReportTemplate(String serviceName,String methodName) throws IOException{

		reportIn = new String(Files.readAllBytes(Paths.get(templatePath)));
		reportIn=reportIn.replace(serviceNamePlaceholder, serviceName+" Service - "+methodName+" test report");
		...
		reportPath = "html_reports/report_" + methodName+"_"+currentDate + ".html";
		//reportPath = "html_reports/" + System.getProperty("ReportName") + ".html";
	}
	public static void storeResultReport(List<TestRow> testRowList) throws IOException {		
		...
		try{
			for (TestRow testRow2 : testRowList) {
				...
				String passedLink="pass"+testRow2.getTestCase();
				testRow2.getRequest();			
				String replaceAllReq = requestModal.replaceAll("headerTC", requestLink).replaceAll("reqbody",
						testRow2.getRequest());
				String replaceAllResp = responseModal.replace("headerTC",responseLink).replace("resbody", testRow2.getResponse());
				...
				String replaceAllFailedError = testRow2.isPassed() ? failedtestModal.replaceAll("headerTC", passedLink).replaceAll("message", testRow2.getFailedError()):failedtestModal.replaceAll("headerTC", failedLink).replaceAll("message", testRow2.getFailedError());
				failedLink="#"+"fail"+testRow2.getTestCase();
				passedLink="#"+"pass"+testRow2.getTestCase();
				String str1 = testRow2.isPassed() ? "<td><a data-toggle=\"modal\" data-target=\""+passedLink+"\"><font color =\"green\">PASSED</font></a></td>"
			...
			System.out.println("Exception Occured in storing report result = " + Ex.getMessage());
		}
